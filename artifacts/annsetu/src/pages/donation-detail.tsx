...
export default function DonationDetail() {
    ...
    claimDonation.mutate(
      { id: donationId },
      {
        onSuccess: () => {
          ...
          toast({ title: "Donation Claimed", description: "You have successfully claimed this donation. Please pick it up before the deadline." });
        },
        onError: () => toast({ variant: "destructive", title: "Error", description: "Could not claim donation." })
      }
    );
  };
    ...
    unclaimDonation.mutate(
      { id: donationId },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: getGetDonationQueryKey(donationId) });
          queryClient.invalidateQueries({ queryKey: getListDonationsQueryKey() });
          toast({ title: "Claim Cancelled", description: "You have cancelled your claim." });
        },
        onError: () => toast({ variant: "destructive", title: "Error", description: "Could not cancel claim." })
      }
    );
  };
  ...
  const handleVerify = () => {
    ...
    verifyPickup.mutate(
      { id: donationId, data: { otp } },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: getGetDonationQueryKey(donationId) });
          toast({ title: "Pickup Verified!", description: "Thank you for completing this donation." });
          setOtp("");
        },
        onError: () => toast({ variant: "destructive", title: "Verification Failed", description: "Invalid OTP. Please check with the NGO/volunteer." })
      }
    );
  };
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-6 pb-12">
      <div className="flex items-center gap-2 text-sm text-muted-foreground cursor-pointer hover:text-foreground" onClick={() => window.history.back()}>
        &larr; Back
      </div>
        ...
        <div className="p-8">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="text-right">
              <div className="text-4xl font-bold text-primary">{donation.quantityPlates}</div>
              <div className="text-sm text-muted-foreground font-medium">Plates Available</div>
            </div>
          </div>

          <div className="grid md:grid-cols-2 gap-8 mb-8">
            <div className="space-y-4">
              <h3 className="font-semibold text-lg flex items-center gap-2 border-b pb-2"><User className="w-5 h-5 text-muted-foreground"/> Donor Info</h3>
              <div className="space-y-2">
                <p className="font-medium text-foreground">{donation.donor?.name || 'Anonymous'}</p>
                <p className="text-sm text-muted-foreground capitalize">{donation.donor?.donorCategory?.replace('_', ' ') || 'Donor'}</p>
                {(donation.status === 'claimed' && (isClaimer || isDonor)) && (
                  <p className="text-sm flex items-center gap-2"><Phone className="w-4 h-4 text-muted-foreground" /> {donation.donor?.phone}</p>
                )}
              </div>
            </div>
            
            <div className="space-y-4">
              <h3 className="font-semibold text-lg flex items-center gap-2 border-b pb-2"><Clock className="w-5 h-5 text-muted-foreground"/> Logistics</h3>
              <div className="space-y-2">
                <p className="text-sm flex items-start gap-2">
                  <MapPin className="w-4 h-4 text-muted-foreground mt-0.5 shrink-0" />
                  <span>{donation.address || 'Address provided upon claim'}</span>
                </p>
          ...
          {donation.description && (
            <div className="mb-8">
              <h3 className="font-semibold text-lg mb-2 border-b pb-2">Instructions</h3>
          ...
          {!isDonor && donation.status === 'available' && (
            <div className="bg-primary/5 p-6 rounded-2xl border border-primary/20 text-center">
              <p className="mb-4 font-medium">Can you pick this up before the deadline?</p>
              <Button size="lg" className="w-full md:w-auto px-8" onClick={handleClaim} disabled={claimDonation.isPending}>
                {claimDonation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Claim Donation
              </Button>
            </div>
          )}
          ...
          {!isDonor && donation.status === 'claimed' && isClaimer && (
            <div className="bg-accent/10 p-6 rounded-2xl border border-accent/20">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h4 className="font-bold text-lg">You claimed this donation</h4>
                  <p className="text-sm text-muted-foreground">Please pick it up from the donor before the deadline. The donor has been notified.</p>
                </div>
              </div>
              <Button variant="outline" onClick={handleUnclaim} disabled={unclaimDonation.isPending}>
                {unclaimDonation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Cancel Claim
              </Button>
            </div>
          )}
            ...
            <div className="bg-primary/5 p-6 rounded-2xl border border-primary/20">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Enter the OTP provided by the volunteer to complete the donation.</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                />
                <Button onClick={handleVerify} disabled={verifyPickup.isPending || otp.length < 4}>
                  {verifyPickup.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Verify & Complete
                </Button>
              </div>

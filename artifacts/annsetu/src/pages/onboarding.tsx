...
const donorSchema = baseSchema.extend({
  donorCategory: z.enum(
    ["restaurant", "hotel", "caterer", "event_org", "household"],
    {
...
function VerifyBadge({
  ...
  return (
    <div className="flex items-center gap-1.5 text-destructive text-sm mt-1">
      <XCircle className="w-3.5 h-3.5" /> Not found in registry. Contact support
      if this is an error.
    </div>
  );
}
  ...
  const handleSubmit = (values: z.infer<typeof donorSchema>) => {
    if (needsLicense && verifyState !== "valid") {
      form.setError("licenseNumber", {
        message: "Please verify your FSSAI License Number before continuing",
      });
      return;
    }
    onSubmit(values);
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                Donor Type <span className="text-destructive">*</span>
              </FormLabel>
              <Select
                onValueChange={(v) => {
                  field.onChange(v);
              ...
              >
                <FormControl>
                  <SelectTrigger>
                    <SelectValue placeholder="What kind of food donor are you?" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Required under Food Safety and Standards Authority of India
                  (FSSAI).
                </p>
                <FormMessage />
              </FormItem>
...
function NgoForm({
  ...
  const handleSubmit = (values: z.infer<typeof ngoSchema>) => {
    if (verifyState !== "valid") {
      form.setError("darpanId", {
        message: "Please verify your Darpan ID before continuing",
      });
      return;
    }
    onSubmit(values);
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="e.g. Feeding India Foundation" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                NITI Aayog Darpan ID <span className="text-destructive">*</span>
              </FormLabel>
              <div className="flex gap-2">
                <FormControl>
                  <Input
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-sm text-blue-800 flex gap-2">
          ...
          <span>
            Your Darpan ID is verified against NITI Aayog records in our
            database. Ensure it matches your registration exactly.
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Your Vehicle <span className="text-destructive">*</span>
                </FormLabel>
        ...
        onSuccess: (result) => {
          if (result.valid) {
            ...
            toast({
              ...
              title: "Invalid code",
              description:
                "This access code is not in the registry or has been deactivated.",
            });
          }
        },
        onError: () => {
  ...
  const handleSubmit = (values: z.infer<typeof adminSchema>) => {
    if (verifyState !== "valid") {
      form.setError("adminCode", {
        message: "Please verify your admin access code first",
      });
      return;
    }
    onSubmit(values);
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                Admin Access Code <span className="text-destructive">*</span>
              </FormLabel>
              <div className="flex gap-2">
                <FormControl>
                  <Input
              ...
              <VerifyBadge
                state={verifyState}
                validMsg="Access code verified — admin access granted"
              />
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button
            type="submit"
            className="flex-1"
            disabled={isPending || verifyState !== "valid"}
          >
            {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            Complete Setup <ChevronRight className="w-4 h-4 ml-1" />
          </Button>
              ...
              {roleCards
                ...
                .map((card) => {
                  ...
                  return (
                    <span
                      key={card.role}
                      className={`flex items-center gap-2 text-sm font-medium ${card.iconColor}`}
                    >

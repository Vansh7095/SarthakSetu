  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-serif font-bold text-foreground">Available Food</h1>
          <p className="text-muted-foreground">Find and claim surplus food nearby.</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Link href="/map">
            <Button variant="outline"><MapPin className="mr-2 h-4 w-4" /> Map View</Button>
          </Link>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {donations.map(donation => (
            <div key={donation.id} className="bg-card border rounded-2xl p-5 flex flex-col gap-4 hover-elevate transition-all">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium">{donation.donor?.name || 'Anonymous Donor'}</p>
                  <p className="text-xs text-muted-foreground capitalize">{donation.donor?.donorCategory?.replace('_', ' ') || 'Donor'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 py-2 border-y border-border">
                <div className="flex flex-col">
                  ...
                  <span className="font-semibold text-lg">{donation.quantityPlates} <span className="text-sm font-normal">plates</span></span>
                </div>
                <div className="flex flex-col">
              ...
              <div className="text-sm text-muted-foreground flex items-start gap-2 mb-2">
                ...
                <span className="line-clamp-2">{donation.address || 'Address provided upon claiming'}</span>
              </div>
        ...
        <div className="bg-card border rounded-xl p-12 text-center flex flex-col items-center">
          ...
          <p className="text-xl font-medium text-foreground">No donations found</p>
          <p className="text-muted-foreground mt-2 max-w-md">There are no available food donations matching your criteria right now. Check back soon!</p>
        </div>
      )}
    </div>
  );

...
function LocationPicker({ lat, lng, address, onPick }: LocationPickerProps) {
  ...
  const handleMapClick = useCallback(async (clickLat: number, clickLng: number) => {
    ...
    mapRef.current?.flyTo([clickLat, clickLng], Math.max(mapRef.current.getZoom(), 14), { animate: true, duration: 0.5 });
  }, [onPick]);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium">Pickup Location <span className="text-destructive">*</span></span>
        <Button
          type="button"
          variant="outline"
          size="sm"
      ...
      {!lat && !lng && !geocoding && (
        <p className="text-xs text-muted-foreground flex items-center gap-1">
          <MapPin className="w-3 h-3" /> Tap the map or use GPS to set your pickup location
        </p>
      )}
...
export default function Donate() {
  ...
  const form = useForm<z.infer<typeof donateSchema>>({
    ...
    defaultValues: {
      ...
      lat: undefined,
      lng: undefined,
      pickupDeadline: new Date(Date.now() + 4 * 60 * 60 * 1000).toISOString().slice(0, 16),
    },
  });
  ...
  const handleLocationPick = (pickedLat: number, pickedLng: number, pickedAddress: string) => {
  ...
  const onSubmit = (values: z.infer<typeof donateSchema>) => {
    if (!values.lat || !values.lng) {
      toast({ variant: "destructive", title: "Location required", description: "Please pin your pickup location on the map." });
      return;
    }
    const payload = {
      ...values,
    ...
    createDonation.mutate(
      { data: payload },
      {
        onSuccess: () => {
          ...
          toast({ title: "Donation Listed", description: "Your food donation is now visible on the map." });
          setLocation("/my-donations");
        },
        onError: () => {
          toast({ variant: "destructive", title: "Error", description: "Failed to create donation. Please try again." });
        },
      }
    );
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Pin your location on the map so NGOs can find you quickly.</p>
      </div>

    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Track the donations you have claimed.
        </p>
      </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {claims.map((claim) => {
            ...
            const deadline = donation.pickupDeadline
              ? new Date(donation.pickupDeadline)
              : null;
            return (
              <div
                key={claim.id}
              ...
              >
                ...
                <div className="text-sm text-muted-foreground flex flex-col gap-2">
                  <span className="flex items-center gap-1">
                    <Clock className="w-4 h-4" /> Pick up by{" "}
                    {deadline
                ...
                {donation.status === "completed" && (
                  <div className="flex items-center gap-2 text-secondary text-sm font-medium mt-2 p-3 bg-secondary/5 rounded-xl border border-secondary/10">
                    <CheckCircle2 className="w-5 h-5" /> Completed on{" "}
                    {claim.completedAt

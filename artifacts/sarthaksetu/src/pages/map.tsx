      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground">
            Find available food donations nearby. Click a pin to see the route.
          </p>
        </div>
        <div className="flex items-center gap-3 text-sm bg-card p-2 rounded-lg border">
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-red-500"></div> Urgent
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-green-500"></div> Household
          </div>
          <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-indigo-500"></div> You
          </div>
          <div className="flex items-center gap-1">
      ...
      {routeDest && (
        <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2 text-sm">
          ...
          <span className="text-blue-800 font-medium">
            Showing route to donation
          </span>
          <Button
            variant="ghost"
            size="sm"
            onClick={clearRoute}
            className="text-blue-700 h-6 text-xs ml-auto"
          >
            Clear route
          </Button>
        </div>
      ...
      <div className="flex-1 rounded-2xl overflow-hidden border border-border shadow-sm relative">
        ...
        >
          ...
          {donations?.map((donation) => {
            ...
            const deadlineTs = donation.pickupDeadline
              ? new Date(donation.pickupDeadline).getTime()
              : 0;
            const now = Date.now();
            const hoursLeft = deadlineTs
              ? (deadlineTs - now) / (1000 * 60 * 60)
              : 999;
            return (
              <Marker
                key={donation.id}
